...
- `slope1() -> uint256`
  File: `contracts/InterestRate.sol`
  Returns: slope used below the kink, scaled by `1e18`.

- `slope2() -> uint256`
  File: `contracts/InterestRate.sol`
  Returns: slope used above the kink, scaled by `1e18`.
...
- All rate values in `InterestRateModel` are per-block values.
