...
- Documented `FlashLoanSwap.setExchangeRate(...)` as the active swap-side configuration entry point.
...
- `setExchangeRate(uint256 _exchangeRate)` (OnlyOwner)
  ...
  Notes: this is the live swap-configuration setter used after deployment; `_exchangeRate` must be greater than `0`.
