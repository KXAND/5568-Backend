...
library DepositLogic {
  ...
  event ClaimAToken(address indexed user, address indexed asset, address indexed to, uint256 shares);
  event RecustodyAToken(address indexed user, address indexed asset, uint256 shares);
  event CollateralDeposited(uint256 indexed debtVaultId, address indexed asset, uint256 amount, uint256 shares);
  ...
  function executeWithdraw(
    ...
    shares = ReserveLogic.sharesFromAssetAmountRoundUp(reserve, amount, ray);
    require(shares > 0, "LendingPool: shares=0");
    require(custodiedShares[user][asset] - lockedShares[user][asset] >= shares, "LendingPool: insufficient claimable shares");
  ...
  function executeClaimAToken(
    ...
    LendingPoolTypes.Reserve storage reserve = reserves[asset];
    custodiedShares[user][asset] -= shares;
    require(reserve.aToken.transfer(to, shares), "LendingPool: aToken transfer failed");
  ...
  function executeRecustodyAToken(
    ...
    IERC20(address(reserve.aToken)).safeTransferFrom(user, address(this), shares);
    custodiedShares[user][asset] += shares;
  ...
  function validateWithdrawCollateral(
    ...
    require(lockedShares[user][asset] >= debtVault.collateralShares[asset], "LendingPool: insufficient locked shares"); 
    require(debtVault.collateralShares[asset] >= shares, "LendingPool: insufficient collateral"); 
  }
}
