...
library DebtVaultLogic {
    ...
    function getLiquidationShares(
        ...
        uint256 liquidatingValue = (repayValue * (bps + liquidationBonus)) /
            bps;
        uint256 collateralAmount = (liquidatingValue * ray) / collateralPrice;

        return
            ReserveLogic.sharesFromAssetAmountRoundUp(
    ...
    function getActualLiquidationTransferredShares(
        ...
        actualTransferredShares = availableShare < requestedShares
            ? availableShare
            : requestedShares;
        actualTransferredShares = lockedShare < actualTransferredShares
            ? lockedShare
        ...
        )
    {
        ...
        // collateral info
        address[] storage collateralAssets = debtVaultCollateralAssets[
            debtVaultId
        ];
        for (uint256 i = 0; i < collateralAssets.length; i++) {
            ...
            uint256 amount = ReserveLogic.assetAmountFromShares(
                reserve,
                shares,
                ray
            );
        ...
        // debt info
        address[] storage borrowedAssets = borrowedAssetsInDebtVault[
            debtVaultId
        ];
        for (uint256 i = 0; i < borrowedAssets.length; i++) {
            ...
            uint256 amount = index == 0
                ? principal
                : (principal * reserve.borrowIndex) / index;
