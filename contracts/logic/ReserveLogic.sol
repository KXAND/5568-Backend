...
library ReserveLogic {
  event Accrue(address indexed asset, uint256 interest, uint256 borrowIndex, uint256 liquidityIndex);
  ...
  function executeAccrueInterest(
    ...
    reserve.borrowIndex = reserve.borrowIndex + (reserve.borrowIndex * borrowRate * blocksElapsed) / ray;
    reserve.liquidityIndex = reserve.liquidityIndex + (reserve.liquidityIndex * supplyRate * blocksElapsed) / ray;
    reserve.lastAccrualBlock = block.number;

    emit Accrue(asset, interest, reserve.borrowIndex, reserve.liquidityIndex);
  }
  ...
  function getUtilization(
    ...
    return (reserve.totalBorrows * ray) / (assetCash + reserve.totalBorrows);
  }
  ...
  function sharesFromAssetAmountRoundUp(
    ...
    return (amount * ray + reserve.liquidityIndex - 1) / reserve.liquidityIndex;
  }
