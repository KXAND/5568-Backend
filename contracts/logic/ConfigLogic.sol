...
library ConfigLogic {
  event ReserveAdded(address indexed asset, address indexed aToken, address indexed interestRateModel);
  ...
  function executeAddReserve(
    ...
    require(params.interestRateModel != address(0), "LendingPool: bad interest rate model");
    require(!isReserveAsset[params.asset], "LendingPool: reserve exists");
    require(params.liquidationThreshold <= ray, "LendingPool: bad threshold");
    require(params.ltv <= params.liquidationThreshold, "LendingPool: ltv > threshold");

    PoolCoin aToken = new PoolCoin(params.aTokenName, params.aTokenSymbol, owner, pool);
    ...
    emit ReserveAdded(params.asset, address(aToken), params.interestRateModel);
